...
export default async function DraftPage() {
  const episode = await getDraft()

  if (!episode) {
    return (
      <div className="text-center py-24">
        <p className="text-2xl font-semibold text-slate-400">No draft pending</p>
        <p className="mt-2 text-slate-500">The pipeline hasn&apos;t generated a new episode yet.</p>
        <Link href="/episodes" className="mt-6 inline-block text-sm text-slate-600 underline">
          View all episodes →
        </Link>
      </div>
    )
  ...
  const timeUntil = publishDate ? formatTimeUntil(publishDate) : null
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-1">
        <div className="flex items-center gap-2 text-sm text-slate-500">
          ...
          {timeUntil && <><span>·</span><span className="text-amber-600 font-medium">{timeUntil}</span></>}
        </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <DraftActions id={episode.id} publishAt={episode.publishAt} />
      </div>
      ...
      {content ? (
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm space-y-6">
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest text-slate-400 mb-3">Article body</p>
